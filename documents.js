...
function renderAdmission(s) {
  return '<h1>TECHO Industrial Automation</h1><h2>Student Admission Form</h2>' +
    ...
    row('Course Name', s.Course) + row('Batch', s.Batch) + row('Admission Date', s.AdmissionDate) + row('Training Mode', 'Online') +
    '<h3>Fee Details</h3>' +
...
function renderInstallment(s) {
  return '<h1>TECHO Industrial Automation</h1><h2>Installment Payment Agreement</h2>' +
    ...
    row('Student Name', s.FullName) + row('Admission No.', s.StudentID) + row('Course', s.Course) +
    row('Total Course Fee', '₹' + s.TotalFee) + row('Fee Paid at Admission', '₹' + s.PaidFee) + row('Balance Fee', '₹' + s.PendingFee) +
    ...
    '<li>I remain responsible for paying the remaining fee even if I leave the course incomplete.</li>' +
...
function renderReceipt(s) {
  return '<h1>TECHO Industrial Automation</h1><h2>Fee Receipt</h2>' +
    ...
    row('Student Name', s.FullName) + row('Admission No.', s.StudentID) + row('Mobile', s.Mobile) + row('Course', s.Course) + row('Batch', s.Batch) +
    '<h3>Fee Details</h3>' +
    row('Total Course Fee', '₹' + s.TotalFee) + row('Total Paid Till Date', '₹' + s.PaidFee) + row('Balance Fee', '₹' + s.PendingFee) +
...
function renderBonafide(s) {
  return '<h1>TECHO Industrial Automation</h1><h2>Bonafide Certificate</h2>' +
    ...
    '<strong>TECHO Industrial Automation</strong> in the <strong>' + (s.Course || '_____________') + '</strong> course, and is / was a regular student of this institute.</p>' +
    row('Admission Date', s.AdmissionDate) + row('Course Status', s.CourseStatus) +
    '<p style="margin-top:16px;">This certificate is issued at the student\'s request for official purposes. This certificate should not be misused for any other purpose.</p>' +
    signBlock('Director / Authorized Signatory');
}

function renderDeclaration(s) {
  return '<h1>Student Declaration Form</h1>' +
    '<p style="margin:16px 0;">I, <strong>' + (s.FullName || '_____________') + '</strong>, am voluntarily taking admission at ' +
    '<strong>TECHO Industrial Automation</strong> in the <strong>' + (s.Course || '_____________') + '</strong> course. I have read and understood the following declarations and agree to them.</p>' +
    '<ol>' +
...
function renderUndertaking(s) {
  return '<h1>TECHO Industrial Automation</h1><h2>Student Undertaking</h2>' +
    ...
    '<strong>' + (s.Course || '_____________') + '</strong> course, and hereby give this undertaking accepting the following:</p>' +
    '<ol>' +
    ...
    '<li>I will behave respectfully with trainers, staff and fellow students.</li>' +
    ...
    '</ol>' +
    row('Admission No.', s.StudentID) + row('Course', s.Course) + row('Mobile', s.Mobile) +
    signBlock();
}
